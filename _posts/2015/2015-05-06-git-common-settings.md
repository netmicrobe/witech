...
git config --global user.email your-email@mail-service.com
...
git config --global core.editor /cygdrive/d/notepad_plusplus_git/npp.sh
git config --global core.filemode false
git config --global color.ui true
git config --global http.sslverify false
git config --global http.postbuffer 524288000
git config --global diff.tool winmerge
git config --global difftool.winmerge.cmd '/cygdrive/where-is-WinMerge/git_diff.sh "$LOCAL" "$REMOTE"'
...
/cygdrive/e/noinstall/notepad_plusplus_git/notepad++.exe -multiInst -noPlugin -nosession "$(cygpath -w "$*")"
```
...
```
git config --unset settings-name
或者
git config --global --unset settings-name
```
